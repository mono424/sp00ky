...
Each machine runs exactly one backend, which needs `deploy.port` and `deploy.healthcheck` and must be `hosting: cloud`. `spky dev` ignores the placement and runs the backend as an ordinary app. Everything else, from what `spky deploy` shows to how a redeploy rolls the VM, is in [Dedicated machines](/docs/cloud/dedicated-machines).
