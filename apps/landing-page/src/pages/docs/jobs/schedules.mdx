...
Release a key by hand once the underlying problem is fixed:

```bash
spky schedules release game-sync 'connection:CONN_abc123'
```

Forgetting the streak **is** the release: the next fire goes through, and if the key is
still broken it quarantines again. `spky schedules get <name>` lists the currently
quarantined keys, as does the admin dashboard.
...
<PropTable props={[
  ...
  { name: 'quarantineAfter', type: 'number', default: '-', desc: 'Consecutive failures of ONE forEach key before it stops firing. Unset means never.' },
