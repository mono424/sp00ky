...
<CodeBlock code={unsyncedCode} lang="dart" />
...
export const unsyncedCode = `// Dart: one tick while the row is only on this device, two once it is stored.
Set<String> unsynced = client.unsyncedRecordIds;
final stop = client.subscribeToUnsyncedRecords((ids) {
  setState(() => unsynced = ids);
});

final stored = !unsynced.contains(conversation.id);
