...
DEFINE FIELD OVERWRITE errors[*] ON TABLE job TYPE object FLEXIBLE;
...
export const jobIndexCode = `DEFINE INDEX IF NOT EXISTS idx_job_dispatch ON TABLE job COLUMNS status, created_at;
DEFINE INDEX IF NOT EXISTS idx_job_activity ON TABLE job COLUMNS updated_at;`;
...
<Note>
  One exception to "the runner is built into the SSP": a backend with [`runOn: { pool }`](/docs/jobs/machine-pools) has its jobs run by the scheduler, on machines of their own. (A backend on a [dedicated machine](/docs/cloud/dedicated-machines) is still driven by the SSP runner.) The job rows, retries, `spky jobs` and the kill and retry commands work the same way.
</Note>
...
An outbox table is the busiest table most projects have, and three things scan
it: the dispatcher's drain, on **every** job completion
(`WHERE status = 'pending' ORDER BY created_at`); the retention prune, which
ages terminal rows by `updated_at`; and the admin dashboard's queue counts. So
`spky deploy` defines two indexes on every outbox table it knows about:

<CodeBlock code={jobIndexCode} lang="surql" />

They are defined with `IF NOT EXISTS`, never `OVERWRITE`, so the build happens
once and every deploy after that is free. `spky add api` writes the same two
lines into a new table's `.surql`, so a fresh project has them from the start.

<Note title="The first deploy after upgrading">
  If your project predates this, the next `spky deploy` builds both indexes,
  and on a table with a lot of rows that is not instant — the deploy will sit on
  the schema step longer than usual. It happens once. A deploy that is already
  close to its timeout can be given more room with
  <code>SPKY_DB_HTTP_TIMEOUT_SECS</code>.
</Note>
