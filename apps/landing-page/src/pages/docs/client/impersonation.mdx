...
export const cssCode = `/* The bar is fixed to the top of the viewport. Offset your own fixed header: */
.app-header {
  top: var(--sp00ky-impersonation-banner-height, 0px);
}`;

Impersonation lets an admin see and use the app exactly as one of its users does, which is the fastest way to reproduce "it looks wrong for me" reports. It is **off unless a project enables it**, it is started from the DevTools **Access** tab, and while it lasts every page shows a red warning bar with a **Stop** button.
...
- **Visible.** The warning bar lives in a closed shadow root and survives app CSS.
...
The warning bar is always shown; you do not have to render anything. It publishes its height as a CSS variable:
