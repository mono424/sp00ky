...
| `SPKY_DRIFT_REPAIR_MAX_ROWS` | Most rows one in-place table repair may send; a bigger difference falls back to a re-clone | `2000` |
| `SPKY_DRIFT_REPAIR_TIMEOUT_SECS` | Deadline for one table repair; past it the repair is abandoned, the upstream connection replaced, and the repair retried on the next check | `300` |
...
| `SPKY_CHANGEFEED_DOORBELL` | `LIVE SELECT id FROM _00_version` over WebSocket as the wake-up for the tail; `false` = poll only. | `true` |
