...
A `forEach` query is re-run from scratch on every fire, so a row that **always** fails is
re-selected forever. Nothing else in the chain notices: `concurrency: skip` only suppresses
a fire while the previous run is still going, and a query that returns a bad row is a
perfectly successful query. One dead row then costs a job every tick indefinitely, and keeps
the outbox's `failed` bucket permanently non-empty — which is exactly where a *real* failure
would have shown.

`quarantineAfter` is the escape hatch.
...
schedules:
  game-sync:
    every: 1m
    forEach:
      query: SELECT id FROM connection WHERE last_synced_at < time::now() - 15m
      key: id
    quarantineAfter: 5    # stop firing a key after 5 consecutive failures
```

Past the budget the key stops firing. Each suppressed fire is recorded as a `skipped` run
carrying `{ code: "quarantine" }`, and the first one opens a `schedule_quarantined` incident
naming the schedule and the key. Every other key of the same schedule is untouched.

A quarantined key is not written off for good. An hour after it was quarantined, one fire
goes through as a **probe**. A probe that fails waits twice as long for the next one (1h, 2h,
4h, 8h, 16h, then every 24h). A probe that succeeds forgets the streak, and the key's next
fire closes the incident. So a dead row costs at most one job a day, and a key whose upstream
only had a bad quarter of an hour recovers without anyone having to notice it first.

Only runs that **actually executed** move the counter. A `skipped` fire is evidence of
nothing — under `concurrency: skip` most of the ticks of a failing key are suppressed, so
counting those either way would defeat the mechanism. One success forgets the streak
entirely.

<Note>
  It is opt-in, and unset means never. Turning a firing behaviour on implicitly at upgrade
  is not a change anyone asked for, so a schedule that wants the budget states it.

  The budget is read at the gate rather than frozen onto a key, unlike `deadline` and
  `history`. Those bound a run already in flight, where a config change must not move the
  goalposts; this decides whether to *start* one at all. Raising or lowering it therefore
  takes effect on the next fire — which is what an operator releasing a stuck key expects.
</Note>

Release a key by hand to skip the wait for its next probe, once the underlying problem is fixed:

```bash
spky schedules release game-sync 'connection:CONN_abc123'
```

Forgetting the streak **is** the release: the next fire goes through and closes the
incident, and if the key is still broken it quarantines again. `spky schedules get <name>`
lists the currently quarantined keys, as does the admin dashboard.
...
<PropTable props={[
  ...
  { name: 'quarantineAfter', type: 'number', default: '-', desc: 'Consecutive failures of ONE forEach key before it stops firing. It is then probed on a doubling backoff (1h up to 24h) and heals on the first probe that succeeds. Unset means never.' },
...
<PropTable props={[
  ...
  { name: 'skipped', type: 'status', default: '-', desc: 'This tick was suppressed. error.code says why: concurrency_skip (the previous run for that key was still going) or quarantine (the key is past its quarantineAfter budget).' },
