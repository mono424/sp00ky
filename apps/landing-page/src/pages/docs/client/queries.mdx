...
All three bypass the query builder, so with the [query allowlist](/docs/guide/query-allowlist)
switched on (`sync.queryAllowlist: warn` or `enforce`) they throw unless the client config sets
[`allowRawRemote: true`](/docs/reference/client-config#allowrawremote).
