...
`drift.auto_repairs` counts in-place table repairs, `drift.auto_reclones` the
full re-clones a repair escalated to. `drift.upstream` is
...
- `SPKY_DRIFT_REPAIR_MAX_ROWS` - Most rows one in-place table repair may send through the ingest pipeline (default: `2000`)
- `SPKY_DRIFT_REPAIR_TIMEOUT_SECS` - Deadline for one table repair; on the deadline it is abandoned, the upstream connection is replaced and the repair is retried on the next check (default: `300`)
...
(its `component` is `schedule:<name>/<key>`). See
