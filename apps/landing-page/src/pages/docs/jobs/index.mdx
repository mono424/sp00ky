...
<Note>
  One exception to "the runner is built into the SSP": a backend with [`runOn`](/docs/jobs/machine-pools) has its jobs run by the scheduler, on machines of their own. The job rows, retries, `spky jobs` and the kill and retry commands work the same way.
</Note>
