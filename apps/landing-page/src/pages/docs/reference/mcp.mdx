...
| `-y`, `--yes` | The [global flag](/docs/reference/cli#running-unattended--y----yes). Non-interactive: read-only unless `--scopes` is given |
...
- **Pools**: `pools_list` (every [machine pool](/docs/jobs/machine-pools) with its sizing, pause and breaker state, machines by state and the jobs waiting for one), `pool_machines` (one pool's machines, newest first, with why each one was taken away), `pool_pause`, `pool_resume`, `machine_drain`
